...
public class Settings extends JFrame {	
	...
	private static void trialSettings() {
		...
		products.add(new Product("DORUÈAK", "Monte Cristo Å unka", 220.0, 100.0));
		products.add(new Product("DORUÈAK", "Monte Cristo Slanina", 260.0, 100.0));
		products.add(new Product("DORUÈAK", "Monte Cristo Kulen", 230.0, 100.0));
		products.add(new Product("DORUÈAK", "Monte Cristo Suvi Vrat", 250.0, 120.0));
		products.add(new Product("DORUÈAK", "Burrito doruèak ", 220.0, 100.0));
		products.add(new Product("DORUÈAK", "Sendviè suvi vrat", 150.0, 100.0));
		...
		products.add(new Product("SOSEVI", "Keèap i majonez", 0.0, 10.0));
	...
	private boolean performCheckNewCategory() {
		...
		if(Libary.checkForCategoryDuplicates(categoryName.toUpperCase())) {
			setUpLabelError("Naziv kategorije veæ postoji.");
			return false;
		}
	...
	private JList getList() {
		if (list == null) {
			...
			list.setModel(new AbstractListModel() {
				String[] values = new String[] {"OPŠTA PODEŠAVANJA" ,"PODEŠAVANJA PROIZVODA", "PODEŠAVANJA KORISNIÈKIH PROFILA"};
				public int getSize() {
	...
	private JPanel getPanelS_display() {
		if (panelS_display == null) {
			...
			JLabel lblDirectory = new JLabel("DIREKTORIJUM IZVEÅ TAJA");
...
//			Icon iconConfirm = rescaleImage(new File("src/lib/img/confirm.png"), 30, 30);
			...
			btnDefault.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					int response = JOptionPane.showConfirmDialog(contentPane, "Da li ste sigurni da želite da restartujete podešavanja na podrazumevana?\n"
							+ "Ovo æe rezultovati brisanju svih podešavanja i povratku na poèetnu šemu.", "Potvrda izlaska", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
	...
	private JPanel getPanelS_products() {
		if (panelS_products == null) {
				...
				public void actionPerformed(ActionEvent e) {
					if(categorySelected) {
						...
						if(!productsOfCategory.isEmpty()) {
							int option = JOptionPane.showConfirmDialog(null, "Da li ste sigurni da želite da obrišete nepraznu kategoriju?\nOvim se brišu svi proizvodi unutar kategorije.", "Brisanje kategorije", JOptionPane.WARNING_MESSAGE);
			...
			JButton btnSaveProduct = new JButton("SAÈUVAJ PROIZVOD");
...
//						Libary.insertProduct(newProduct);
			...
			panelNewCategory.setBorder(borderNewCategory);
						...
						JButton btnSaveCategory = new JButton("SAÈUVAJ");
