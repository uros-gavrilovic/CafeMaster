...
//	}
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent arg0) {
				if(changesSaved == false) {
					int response = JOptionPane.showConfirmDialog(contentPane, "Da li ste sigurni da želite da izaðete? Sve unete izmene neæe biti saÄuvane u izveštaj.", "Potvrda izlaska", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
	...
	private JButton getBtnNaplatiSto() {
		if (btnNaplatiSto == null) {
			...
			btnNaplatiSto.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if(selectedTable == null) {
						JOptionPane.showMessageDialog(contentPane, "Molimo izaberite sto za koji želite da naplatite raÄun", "Naplata raÄuna", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					if(selectedTable.getTableBill() == 0.0) {
						JOptionPane.showMessageDialog(contentPane, "Trenutni sto nema potraživanja i nije moguæe naplatiti raÄun.", "Naplata raÄuna", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
	...
	private JMenuItem getMnItemCreateReport() {
		if (mnItemCreateReport == null) {
			...
			mnItemCreateReport.setToolTipText("Kreira dnevni izveštaj koji prikazuje finansijsku analizu i koliÄinu svakog proizvoda koji je naruÄen.");
			...
			mnItemChangesSaved.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(report.exists() == true) {
						if(changesSaved == false) {
							JOptionPane.showMessageDialog(contentPane, "Izveštaj je napravljen, ali nove promene nisu saÄuvane.", "Uvid o postojanju izveštaja", JOptionPane.INFORMATION_MESSAGE);
						} else {
							JOptionPane.showMessageDialog(contentPane, "Izveštaj je napravljen i kompletno saÄuvan.", "Uvid o postojanju izveštaja", JOptionPane.INFORMATION_MESSAGE);
						}
					} else {
	...
	private JMenuItem getMnItemSettings() {
		if (mnItemSettings == null) {
			...
			mnItemSettings.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {	
					if(changesSaved == false) {
						int response = JOptionPane.showConfirmDialog(contentPane, "Da li ste sigurni da želite da izaðete? Sve unete izmene neæe biti saÄuvane u izveštaju.", "Potvrda izlaska", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
						if(response == JOptionPane.YES_OPTION) {
							goToSettings();
	...
	private JMenuItem getMnItemBack() {
		if (mnItemBack == null) {
			...
			mnItemBack.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {		
					if(changesSaved == false) {
						int response = JOptionPane.showConfirmDialog(contentPane, "Da li ste sigurni da želite da izaðete? Sve unete izmene neæe biti saÄuvane.", "Potvrda izlaska", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
						if(response == JOptionPane.YES_OPTION) {
							goToMainMenu();
							dispose();
						}
	...
	private JMenuItem getMnItemCreateLogs() {
		if (mnItemCreateLogs == null) {
			...
			mnItemCreateLogs.setToolTipText("Kreira evidenciju koja pruža uvid o naruÄenim proizvodima kroz vreme.");
		}
	...
	private JPanel getPanelNW() {
		if (panelNW == null) {
			...
			panelNW.setToolTipText("Ime korisnika na Äijem profilu se obavlja poslovna aktivnost.");
